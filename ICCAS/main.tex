...
\vspace{1.5mm}
\caption{\label{fig_sim_nav2}Qualitative obstacle-aware replanning in the reconstructed virtual environment. The simulated quadruped robot runs in Isaac Sim (left), while RViz shows Nav2 path planning and local costmap updates around inserted obstacles (right).}
\end{center}
\end{figure}
...
The resulting mapping outputs generated during real-world deployment are shown in Fig.~\ref{fig_real_mapping}.
...
\vspace{1.5mm}
\caption{\label{fig_real_mapping}Visual mapping results from real-world deployment. Left: Reconstructed 3D point cloud map from RTAB-Map. Right: Projected 2D occupancy grid map.}
\end{center}
\end{figure}
...
The main contribution of this work is not the direct transfer of a learned locomotion controller, but the construction of a practical validation pipeline for transferring visual navigation and mapping parameters from simulation to a physical quadrupedal platform. These results suggest that 3DGS-based digital twins can serve as a useful sandbox for evaluating RGB-D SLAM and navigation behavior under legged locomotion-induced camera motion.
