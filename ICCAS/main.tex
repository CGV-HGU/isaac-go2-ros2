...
Photorealistic reconstruction methods provide another route to reducing the perceptual sim-to-real gap. Structure-from-Motion pipelines such as COLMAP \cite{ref9} estimate camera poses, while 3DGS \cite{ref4} enables real-time photorealistic rendering from reconstructed scenes. Although frameworks like \mbox{VR-Robo} \cite{ref5} use 3DGS to build digital twins, they often rely on heterogeneous sensors, introducing device-level discrepancies in lens distortion and field-of-view (FOV). In contrast, our pipeline configures the virtual camera to match the physical D435i sensor, minimizing visual discrepancies.
