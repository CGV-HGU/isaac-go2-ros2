...
\abstract{
    Directly deploying autonomous navigation systems on physical quadruped robots in real-world environments presents significant challenges, particularly regarding hardware safety, time-inefficient trials, and the dynamic camera "bobbing noise" (shaking) inherent to legged locomotion that severely degrades RGB-D visual feature tracking and depth consistency. To address these limitations, this paper proposes a practical, dual-stage sim-to-real framework for RGB-D camera-based visual navigation that splits virtual validation and physical deployment. In the simulation stage, we reconstruct a high-fidelity 3D Gaussian Splatting (3DGS) digital twin of an indoor corridor and employ a reinforcement learning (RL) locomotion policy to actively mimic the camera shaking dynamics. Under this realistic noise, the parameters of the on-board RTAB-Map RGB-D Visual SLAM and Nav2 stack are safely pre-tuned and optimized. In the deployment stage, the computationally heavy RL policy is replaced by the robot's built-in Sport API via a lightweight ROS 2 bridge to fit the strict embedded constraints of the Jetson Orin NX. Experimental results demonstrate that parameters tuned under the simulated bobbing noise transfer robustly to the physical Unitree Go2 robot, achieving stable visual mapping and autonomous obstacle avoidance without real-world parameter retuning.
}
...
However, implementing visual navigation utilizing a single RGB-D camera on top of quadrupedal locomotion introduces two critical sim-to-real challenges. First is the \textit{perception-level sim-to-real gap}: standard simulators lack realistic visual textures and depth details, causing RGB-D feature-based SLAM algorithms to fail immediately upon deployment due to poor feature matching. Second is the \textit{locomotion-induced bobbing noise}: unlike wheeled robots, legged walking generates high-frequency camera shaking and wobbling. This motion noise causes severe motion blur in the RGB stream, destroying visual odometry feature tracking, and creates dynamic errors in the depth measurements, leading to false obstacles in the navigation costmap. Tuning navigation stack parameters to withstand these effects directly on physical hardware is highly time-inefficient and poses a constant risk of severe hardware damage from collisions. 
...
\begin{enumerate}
    \item We mitigate the perception-level sim-to-real gap by reconstructing high-fidelity 3DGS digital twins that provide photorealistic RGB textures and consistent depth details for RGB-D visual SLAM validation;
    ...
    \item We establish a highly practical, low-cost visual navigation guide by using a lightweight ROS 2-to-Sport API bridge on a single Jetson Orin NX and a single RGB-D camera, achieving zero-shot transfer of parameters tuned under simulated bobbing noise.
\end{enumerate}
...
The autonomous navigation system is designed as a budget-friendly, LiDAR-free visual pipeline consisting of an Intel RealSense D435i RGB-D camera, RTAB-Map, and the Nav2 framework. The D435i camera is mounted on the front of the Unitree Go2. In the virtual validation environment, a simulated RGB-D sensor generates matching color and depth streams.

Our visual navigation logic relies entirely on the synchronized RGB-D streams. First, RTAB-Map processes the RGB stream to extract visual keypoint features (e.g., GFTT/ORB) for keyframe-based visual odometry (VO), loop-closure detection, and pose-graph optimization. The aligned depth stream is then used to project these visual features into 3D coordinates, building a local 3D octomap, which is subsequently projected into a 2D occupancy grid map. Second, to achieve real-time, low-latency obstacle avoidance without a physical LiDAR sensor, the raw depth image is converted into a 2D laser scan format using the \texttt{depthimage\_to\_laserscan} ROS 2 node. This virtual laser scan is published as a \texttt{/scan} topic to feed the Nav2 local costmap. Nav2's global planner calculates paths on the occupancy grid map, while the Dynamic Window Approach B (DWB) local planner uses the local costmap containing the virtual scan to compute velocity commands ($\mathbf{v}_{cmd}$) to steer the robot around obstacles. Finally, the velocity commands are routed to the locomotion controller.
