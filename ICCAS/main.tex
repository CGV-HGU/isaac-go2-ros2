...
A key technical challenge in establishing the simulation-based validation sandbox is resolving the coordinate transform (TF) tree discrepancy between the physics simulator and the standard ROS 2 navigation stack. Conventionally, Nav2 expects a transform chain of $\texttt{map} \rightarrow \texttt{odom} \rightarrow \texttt{base\_link}$ (or $\texttt{base}$). In contrast, Isaac Sim defines and tracks the robot's pose relative to the simulation world origin ($\texttt{world}$ frame). To satisfy both environments simultaneously and prevent coordinate conflicts or loops, we implement a custom static and dynamic transform bridge. The resulting TF tree is structured as $\texttt{map} \rightarrow \texttt{odom} \rightarrow \texttt{world} \rightarrow \texttt{base}$. Here, RTAB-Map publishes the drift-corrected $\texttt{map} \rightarrow \texttt{odom}$ transform, while Isaac Sim's OmniGraph publisher links the odometry to the simulator's global frame, and the robot's state publisher completes the transform to the robot's physical base. This unique transform chain enables seamless compatibility, allowing standard ROS 2 navigation algorithms to operate within the virtual sandbox without code modification.
